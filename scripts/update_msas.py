...
@click.option(
    "--gene-list",
    help="List of genes to update the MSAs.",
...
def main(
    ...
    if gene_list is None:
        ...
        logging.info("Getting MSAs from gene list...")

        with open(gene_list) as gene_list_fh:
            genes = [gene.strip() for gene in gene_list_fh]

        for gene in genes:
            msa_lookup[gene] = Path(f"{msa_dir}/{gene}.fa")

        # TODO: this is just for logging, remove me
        logging.info(f"msa_lookup = {msa_lookup}")
        logging.info(f"Found {len(msa_lookup)} MSA files.")
